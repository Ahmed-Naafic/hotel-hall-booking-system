...
|---|---|---|---|
| 1.11 | 2026-09-08 | Ahmed | **Post-completion amendment, `BDR-019`.** `POST /api/v1/auth/register` (WBS-08's endpoint) now also requires `fullName` for a `HOTEL_MANAGER` registration, stored directly on the User row (no separate Manager profile table) and echoed on the registration response — Technical Design v1.12 documents the change and its rationale (a Hotel Manager's own identity should be as real as a Customer's, and Admin/Manager screens can now display it instead of only a raw user id). No WBS task is reopened; additive behavior on an already-`Done` endpoint, the same category of change §9 anticipates. `FE-06` (Hotel Manager Register/Login) gains a Full Name field, mirroring `FE-03`'s `BDR-018` amendment. |
