...
Finder _submitButton() => find.widgetWithText(ElevatedButton, 'Save & Continue');
...
/// request itself.
Future<http.Response> _withMediaStub(http.Request r, Future<http.Response> Function(http.Request) handler) {
...
Widget _wrap(
  ...
  final hotelContext = HotelContextController(repository: HotelRepository(apiClient), storage: InMemoryTokenStorage())
    ..hotel = Hotel.fromJson(_hotelJson(status: hotelStatus, profileData: existingProfileData));
  return MultiProvider(
    providers: [
      Provider<ApiClient>.value(value: apiClient),
      ChangeNotifierProvider<HotelContextController>.value(value: hotelContext),
    ],
    child: MaterialApp(home: HotelProfileFormScreen(pickImage: pickImage)),
  );
}
...
void main() {
  ...
  group('Required field validation (BR-HOTEL-02, BDR-015)', () {
    testWidgets('submitting with nothing filled shows all four required-field errors and never calls the API', (tester) async {
      var called = false;
    ...
    testWidgets('Email has no required-field validator — no error shown when left blank', (tester) async {
      await tester.pumpWidget(_wrap((r) async => successResponse(_hotelJson(status: 'PROFILE_COMPLETE'))));
  ...
  group('Saving the profile', () {
    testWidgets('all four required fields plus optional email are sent as an unwrapped PATCH body', (tester) async {
      Map<String, dynamic>? sent;
      late HotelContextController capturedContext;
      final apiClient = ApiClient(
        httpClient: MockClient((r) => _withMediaStub(r, (r) async {
      ...
      await tester.pumpWidget(MultiProvider(
        ...
        child: const MaterialApp(home: HotelProfileFormScreen()),
      ));

      await _fillRequiredFields(tester);
      await tester.enterText(find.widgetWithText(TextFormField, 'Email (optional)'), 'contact@grandhall.example');
      await _tapSubmit(tester);
      ...
      expect(body, contains('"location":"Nairobi, Kenya"'));
    ...
    testWidgets('leaving Email blank omits the email key entirely (never sent as an empty string)', (tester) async {
      Map<String, dynamic>? sent;
      await tester.pumpWidget(_wrap((r) async {
        sent = {'body': r.body};
  ...
  group('Existing profile data', () {
    testWidgets('pre-fills existing standard fields and existing custom fields separately', (tester) async {
      await tester.pumpWidget(_wrap(
        (r) async => successResponse(_hotelJson(profileData: {
          'name': 'Original Name',
  ...
  group('Custom (Additional Information) fields', () {
    testWidgets('a custom field is sent alongside the standard fields', (tester) async {
      Map<String, dynamic>? sent;
      await tester.pumpWidget(_wrap((r) async {
        sent = {'body': r.body};
        return successResponse(_hotelJson(status: 'PROFILE_COMPLETE'));
      ...
      await _fillRequiredFields(tester);
      await tester.enterText(find.widgetWithText(TextFormField, 'Field name'), 'amenities');
      await tester.enterText(find.widgetWithText(TextFormField, 'Value'), 'Pool, Spa');
      await _tapSubmit(tester);
    ...
    testWidgets('an empty custom-field row is silently skipped, not sent as an empty key', (tester) async {
      Map<String, dynamic>? sent;
      await tester.pumpWidget(_wrap((r) async {
        sent = {'body': r.body};
        return successResponse(_hotelJson(status: 'PROFILE_COMPLETE'));
    ...
    testWidgets('a custom field reusing a reserved standard name is rejected — never sent, never substitutes for the real field', (tester) async {
      var called = false;
      await tester.pumpWidget(_wrap((r) async {
        called = true;
        return successResponse(_hotelJson(status: 'PROFILE_COMPLETE'));
      ...
      await _fillRequiredFields(tester);
      await tester.enterText(find.widgetWithText(TextFormField, 'Field name'), 'name');
      await tester.enterText(find.widgetWithText(TextFormField, 'Value'), 'A sneaky override');
      await _tapSubmit(tester);

      expect(called, false);
      expect(find.textContaining('cannot reuse a standard field name'), findsOneWidget);
    });
  });

  group('API failures', () {
    testWidgets('an unauthorized (401) API failure is shown and the form stays open', (tester) async {
      await tester.pumpWidget(_wrap(
        (r) async => errorResponse('AUTHENTICATION_ERROR', 'Missing, invalid, or expired access token.', 401),
      ));

      await _fillRequiredFields(tester);
      await _tapSubmit(tester);

      expect(find.text('Missing, invalid, or expired access token.'), findsOneWidget);
      expect(find.byType(HotelProfileFormScreen), findsOneWidget);
    });

    testWidgets('a cross-tenant/not-found (404) API failure surfaces the server message without crashing', (tester) async {
      await tester.pumpWidget(_wrap(
        (r) async => errorResponse('NOT_FOUND', 'Not found, or belongs to a different Hotel Manager.', 404),
      ));

      await _fillRequiredFields(tester);
      await _tapSubmit(tester);

      expect(find.text('Not found, or belongs to a different Hotel Manager.'), findsOneWidget);
    });
  });

  group('Own-Hotel scoping', () {
    testWidgets('the PATCH always targets the Hotel resolved by HotelContextController, never a manually-entered id', (tester) async {
      Map<String, dynamic>? sent;
      await tester.pumpWidget(_wrap((r) async {
  ...
  group('Hotel Media (BDR-015, ADR-0006)', () {
    Widget wrapWithMediaHandler(Future<http.Response> Function(http.Request) handler, {ImagePickerFn? pickImage}) {
      ...
      return MultiProvider(
        ...
        child: MaterialApp(home: HotelProfileFormScreen(pickImage: pickImage ?? _defaultTestPicker)),
      );
    }

    testWidgets('shows "Upload Logo" when no logo exists yet, and uploading switches it to "Replace Logo"', (tester) async {
      var uploadCalled = false;
      await tester.pumpWidget(wrapWithMediaHandler((r) async {
        if (r.method == 'GET') return successResponse({'logo': null, 'photos': []});
        uploadCalled = true;
        return successResponse({
          'id': 'logo1',
          'hotelId': 'h1',
          'type': 'LOGO',
          'url': 'https://mock-storage.local/hotel-media/hotels/h1/logo/logo1.jpg',
    ...
    testWidgets('loads and displays an existing logo and photos', (tester) async {
      await tester.pumpWidget(wrapWithMediaHandler((r) async => successResponse({
            'logo': {
              'id': 'logo1',
              'hotelId': 'h1',
              'type': 'LOGO',
            ...
            'photos': [
              {
                ...
                'hotelId': 'h1',
                'type': 'PHOTO',
                'url': 'https://mock-storage.local/hotel-media/hotels/h1/photos/p1.jpg',
                'createdAt': '2026-08-26T00:00:00.000Z',
                'updatedAt': '2026-08-26T00:00:00.000Z',
              },
            ],
    ...
    testWidgets('deleting the logo removes it and reverts the button to "Upload Logo"', (tester) async {
    ...
    testWidgets('an upload API failure shows the server message', (tester) async {
      await tester.pumpWidget(wrapWithMediaHandler((r) async {
        if (r.method == 'GET') return successResponse({'logo': null, 'photos': []});
        return errorResponse('VALIDATION_ERROR', 'Unsupported file type. Allowed formats: JPEG, PNG, WebP.', 400);
      }));
      await tester.pump();
      ...
      expect(find.text('Unsupported file type. Allowed formats: JPEG, PNG, WebP.'), findsOneWidget);
    });

    testWidgets('cancelling the picker never sends an upload request', (tester) async {
      var called = false;
      await tester.pumpWidget(wrapWithMediaHandler(
        (r) async {
          called = true;
